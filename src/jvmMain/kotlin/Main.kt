...
import TCPServer.Accepter
import TCPServer.BSDSockAddrIN
import TCPServer.SockAddr
import TCPServer.SockAddrIN
...
public object TCPServer {
    public val libnames: Array<String?> = if (Platform.getNativePlatform().getOS() == Platform.OS.SOLARIS)
        arrayOf<String?>("socket", "nsl", "c")
    else
        arrayOf<String?>(Platform.getNativePlatform().getStandardCLibraryName())
    private val libc: LibC = Library.loadLibrary<LibC>(LibC::class.java, *libnames)
    private val runtime: Runtime = Runtime.getSystemRuntime()

    public fun htons(value: Short): Short {
        return (((value.toInt() and 0xFF) shl 8) or ((value.toInt() and 0xFF00) ushr 8)).toShort()
    }

    public fun serverSocket(port: Int): NativeServerSocketChannel {
    ...
    public open class SockAddr : Struct(runtime)
